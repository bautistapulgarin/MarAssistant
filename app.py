...
<div style="position:fixed; bottom:3.3%; left:15%; font-size:25px; opacity:1; animation:floatY 3s ease-in-out infinite; z-index:9999;">🦌</div>
<div style="position:fixed; bottom:3.3%; left:13%; font-size:25px; opacity:1; animation:floatY 2.8s ease-in-out infinite; z-index:9999;">🦌</div>
<div style="position:fixed; bottom:3.3%; left:17%; font-size:27px; opacity:1; animation:floatY 3.2s ease-in-out infinite; z-index:9999;">🦌</div>
...
# ==============================
# FUNCIONES PARA PROCESAR RESTRICCIONES DE CONTRATO
# ==============================
def procesar_restriccion_contrato(texto):
    """Procesa textos que siguen la estructura de restricción de contrato"""
    texto_lower = texto.lower()
    
    # Patrón para detectar la estructura de restricción de contrato
    patrones = [
        r"restriccion contrato en (.+)",
        r"restricción contrato en (.+)",
        r"restriccion de contrato en (.+)",
        r"restricción de contrato en (.+)"
    ]
    
    for patron in patrones:
        match = re.search(patron, texto_lower)
        if match:
            datos_extraidos = match.group(1)
            return extraer_campos_restriccion(datos_extraidos)
    
    return None

def extraer_campos_restriccion(texto_datos):
    """Extrae los 4 campos del texto usando diferentes estrategias"""
    # Estrategia 1: Buscar por separadores "en" o "para"
    partes = re.split(r'\s+(?:en|para|del|de la|de)\s+', texto_datos, flags=re.IGNORECASE)
    
    if len(partes) >= 4:
        return {
            'proyecto': partes[0].strip(),
            'componente': partes[1].strip(),
            'acuerdo_servicio': partes[2].strip(),
            'detalle': partes[3].strip()
        }
    
    # Estrategia 2: Buscar por el patrón específico de campos
    patron_campos = r"(.+?)\s*\+\s*(.+?)\s*\+\s*(.+?)\s*\+\s*(.+)"
    match = re.search(patron_campos, texto_datos)
    if match:
        return {
            'proyecto': match.group(1).strip(),
            'componente': match.group(2).strip(),
            'acuerdo_servicio': match.group(3).strip(),
            'detalle': match.group(4).strip()
        }
    
    # Estrategia 3: Si no se pueden extraer 4 campos claros, usar el texto completo como detalle
    return {
        'proyecto': 'No especificado',
        'componente': 'No especificado', 
        'acuerdo_servicio': 'No especificado',
        'detalle': texto_datos.strip()
    }

def mostrar_tabla_resumen_restriccion(datos_restriccion):
    """Muestra una tabla resumen con los 4 campos de la restricción"""
    if not datos_restriccion:
        return None
    ...
    df_resumen = pd.DataFrame([
        {
            'Campo': 'Proyecto',
            'Valor': datos_restriccion['proyecto']
        },
        {
            'Campo': 'Componente', 
            'Valor': datos_restriccion['componente']
        },
        {
            'Campo': 'Acuerdo de Servicio',
            'Valor': datos_restriccion['acuerdo_servicio']
        },
        {
            'Campo': 'Detalle',
            'Valor': datos_restriccion['detalle']
        }
    ])
...
def guardar_formulario():
    ...
    # Crear el resumen concatenado
    resumen = f"{proyecto} + {componente} + {acuerdo_servicio} + {detalle_opcional}"
    ...
    st.session_state.datos_guardados = {
        ...
        'detalle_opcional': detalle_opcional,
        'resumen': resumen  # Agregar el resumen
    }
    
    # Mostrar mensaje de éxito con el resumen
    st.success(f"✅ Datos guardados correctamente!")
    st.info(f"📋 **Resumen del registro:** {resumen}")
    
    # Pequeña pausa para mostrar el mensaje
    time.sleep(2)  # Aumenté a 2 segundos para que se pueda leer el resumen
    cerrar_modal()
...
if excel_loaded:
    ...
    def generar_respuesta(pregunta):
        ...
        # 🆕 NUEVO BLOQUE: Procesamiento de restricciones de contrato con estructura específica
        if any(term in pregunta_norm for term in ["restriccion contrato", "restricción contrato"]):
            datos_restriccion = procesar_restriccion_contrato(pregunta)
            if datos_restriccion:
                df_resumen = mostrar_tabla_resumen_restriccion(datos_restriccion)
                if df_resumen is not None:
                    # Guardar en session_state para uso posterior si es necesario
                    st.session_state.ultima_restriccion_procesada = datos_restriccion
                    return f"📋 Resumen de Restricción de Contrato", df_resumen, None, 'restriccion_contrato', None
...
if st.session_state.current_view == 'predictor':
    ...
    st.markdown(
        ...
        '<p style="margin:0 0 0 0;">Escribe tu consulta relacionada con el estado u contexto de los proyectos. Ej: "restricciones de materiales en Burdeos" o "Restriccion contrato en Proyecto + Componente + Acuerdo + Detalle"</p></div>',
        unsafe_allow_html=True
    )
    ...
    with st.form("query_form", clear_on_submit=False):
        ...
        with col_input:
            pregunta = st.text_input(label="", placeholder="Ej: 'Avance de obra en proyecto Altos del Mar' o 'Restriccion contrato en El Castell Iberia + Torre 1 + Provisional electrico + Falta aprobación'", label_visibility="collapsed", key='chat_query')
