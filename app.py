...
CARGOS_VALIDOS}
    ...
    def generar_respuesta(pregunta):
        ...
        # 🎯 Bloque de Restricciones (¡AQUÍ ESTÁ LA CORRECCIÓN CLAVE!)
...
margin-top: 20px; box-shadow: var(--shadow-light);">
            <p style="font-size: 18px; font-weight: 500; color: #34495e;
margin-bottom: 5px;">Resultado de la Predicción:</p>
            <div style="display:flex; align-items:center; gap: 20px;">
                <p style="font-size: 50px; font-weight: 900; color: {color_res};
margin: 0;">{prob_percent:.1f}%</p>
                <div>
                    <p style="font-size: 24px; font-weight: 700; color: #1b2635;
margin: 0 0 5px 0;">{icono} {estado}</p>
                    <p style="font-size: 15px; color: #6b7280; margin: 0;">Probabilidad
de que el contrato se cumpla a tiempo.</p>
                </div>
            </div>
        </div>
        """, unsafe_allow_html=True)
...
for message in st.session_state.messages:
    with st.chat_message(message["role"]):
        if message["type"] == "df":
            ...
            st.markdown(f'<p style="font-size: 18px; font-weight: 600;
color:{PALETTE["primary"]}; margin-bottom: 10px;">{message["title"]}</p>',
unsafe_allow_html=True)
            if message["graph"]:
